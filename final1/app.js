...
var content = fs.readFileSync('/home/ubuntu/workspace/data/syllabus10.txt');
...
// async.eachSeries(meetings, function(value, callback) { // eachSeries in the async module iterates over an array and operates on each item in the array in series

//             var apiRequest = 'https://maps.googleapis.com/maps/api/geocode/json?address=' + value.split(' ').join('+') + '&key=' + apiKey; // requesting API

//             meetings.address = value;
...
//                     console.log("ZERO RESULTS for" + meetings.address);
//                 }
//                 else {

//                  meetings.latLong = JSON.parse(body).results[0].geometry.location;
//                 }
//             });

//             setTimeout(callback, 1000); //slowing downthe results so that the API rules are met

// },function() {
//     console.log(meetings);
    fs.writeFileSync('/home/ubuntu/workspace/data/manhattan10.txt', JSON.stringify(meetings));
//});
